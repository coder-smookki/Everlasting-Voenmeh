...
$ robotcolor = "#999999"
#Если робот собран до конца
label robot_is_completed_defence:
    #Председатель комиссии:
    show down_box("Председатель комиссии","#FFFFFF",t="Молодой человек, ваш робот выглядит впечатляюще. Но главный вопрос: какую практическую пользу он может принести?")

    #Марк:
    show gg_day3 happy
    show down_box("Марк","#B6544D",t="Спасибо за вопрос. Мой робот предназначен для мониторинга и диагностики оборудования в труднодоступных местах. Например, на производственных линиях или в вентиляционных системах.")

    #Второй член комиссии:
    show down_box("Второй член комиссии","#FFFFFF",t="И как он справляется с непредвиденными препятствиями?")

    #Марк:
    show gg day3 calm
    show down_box("Марк","#B6544D",t="Продемонстрируй обход препятствий.")

    #робот
    show down_box("робот","#FFFFFF",t="(Робот плавно объезжает внезапно поставленный на его пути стул, затем возвращается на позицию.)- И")

    #Третий член комиссии:
    show down_box("Третий член комиссии","#FFFFFF",t="А энергопотребление? В записке указано 8 часов автономной работы.")

    #Марк:
    show gg day3 happy 2
    show down_box("Марк","#B6544D","#FFFFFF",t="Это подтверждено тестами. Использованы литий-полимерные аккумуляторы и система оптимизации энергозатрат. Робот может работать полную рабочую смену без подзарядки.")

    #Председатель комиссии:
    show down_box("Председатель комиссии","#FFFFFF",t="Что насчёт расширяемости функционала?")

    #Марк:
    show down_box("Марк","#B6544D","#FFFFFF",t="Конструкция модульная. Можно устанавливать дополнительное оборудование — тепловизоры, газоанализаторы, манипуляторы. Это делает робота универсальной платформой для разных задач.")

    "(После успешной демонстрации всех функций)"

    #Председатель комиссии:
    show down_box("Председатель комиссии","#FFFFFF",t="(обращаясь к комиссии)Коллеги, я считаю, мы увидели качественную проработанную работу. Соответствует всем заявленным характеристикам.")

    #коммисия
    show down_box("коммисия","#FFFFFF",t="Комиссия единогласно ставит высший балл.")
    ...
    #Марк: 
    show gg day3 happy3
    show down_box("Марк","#B6544D",t="Знаете, что самое удивительное? Он продолжает совершенствоваться. Анализирует свои действия и оптимизирует алгоритмы.")

    #Робот:

    show down_box("Робот","#FFFFFF",t="Все текущие задачи выполнены. Готов к новым.")

    #Лев:

    show down_box("Лев","#B99036",t="Повезло тебе с аккумулятором от Жаровой. Без него этот проект так и остался бы на бумаге.")
    
    #Алиса:
    show down_box("Алиса","#DA4E8B",t="Теперь главное — найти ему достойное применение. Думаю, у нас всё получится.")

    #(на фоне мастерской формат И)
    #Система:

    "Конец. Диплом успешно защищен. Робот завершён и готов к работе. Ваше упорство и внимание к деталям принесли заслуженный успех."

#Если робот собран не до конца
label robot_is_not_completed_defence:
    #(фон- какой-нибудь светлый комп класс)

    #Марк:
    show down_box("Марк","#B6544D","(пытаясь говорить уверенно)
Уважаемые члены комиссии, представляю вашему вниманию автономного мобильного робота, способного...")

    #Председатель комиссии
    show down_box("Председатель комиссии (кто-то из руководства кого не юзали)",t="(прерывая) Молодой человек, почему ваш «автономный» робот подключен к розетке?")

    show down_box("Марк","#B6544D",t="Ну... это временная мера... для демонстрации...")

    show down_box("Второй член комиссии:","#FFFFFF",t= "(листая папку) В пояснительной записке вы утверждали, что аппарат способен работать 8 часов без подзарядки. Вы понимаете, что это введение в заблуждение?")

    show down_box("Марк","#B6544D",t= "Я... не рассчитал энергопотребление...")

    show down_box("Третий член комиссии:","#FFFFFF",t="(строго)
И это при том, что основы схемотехники и расчёта нагрузки вы проходили на втором курсе. Продолжайте.")

    show down_box("Марк","#B6544D",t="(пытается включить демонстрацию)
Итак, робот способен распознавать объекты и...")

    "(Робот делает рывок, мигает и внезапно отключается, несмотря на подключенный кабель. В аудитории повисает неловкая тишина.)"

    show down_box("Председатель комиссии:","#FFFFFF",t="(холодно)
Видимо, и с питанием от сети есть проблемы. Ваша работа не просто не соответствует заявленным характеристикам — она демонстрирует полное пренебрежение к базовым инженерным принципам.
")
    show down_box("Второй член комиссии:","#FFFFFF",t="(откладывает ручку)
Мистер Марк, университет не может аттестовать специалиста, неспособного выполнить элементарные расчёты и собрать работоспособный прототип.")

    show down_box("Председатель комиссии:","#FFFFFF",t="(объявляет решение)
Комиссия единогласно постановляет: работа не принимается. Вопрос о вашем дальнейшем пребывании в университете будет решаться на академическом совете. Скорее всего, вас отчислят.")

    #(фон чисто черный, на нем текст концовки)
    #Конец. Неудачная защита диплома привела к отчислению. Возможно, в следующий раз стоит уделить больше внимания технической реализации проекта.
