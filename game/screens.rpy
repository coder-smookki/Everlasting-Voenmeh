...
# Переменная состояния
default heroes_hover = False

# Герои
transform heroes_normal:
    linear 0.3 zoom 1.0 yalign 1.0 yoffset 0  # нижний край героев на границе экрана

transform heroes_hovered:
    linear 0.3 zoom 0.95 yalign 1.0 yoffset 30  # уменьшаются вниз

# Текст
transform text_normal:
    linear 0.3 zoom 1.0 yalign 0.15 yoffset 0  # текст на исходной позиции

transform text_hovered:
...
screen main_menu():
    ...
    # Текст
    add im.Scale("gui/main_text.png", 1000, 200) xalign 0.5 at (text_hovered if heroes_hover else text_normal)

    # Герои
    add "gui/main_menu_heroes.png" xalign 0.5 at (heroes_hovered if heroes_hover else heroes_normal)
    ...
    imagebutton:
        ...
        unhovered SetVariable("heroes_hover", False)
